...
<title>My JSP 'updatetraincar.jsp' starting page</title>
...
<%
	try {
		...
		List<Traincar> list = traincarDao.getOne(cnum);
		List<Traincar> allcar = traincarDao.getAlls();
		List<Car> car = carDao.getAll();

		for (int j = 0; j < car.size(); j++)
			for (int i = 0; i < allcar.size(); i++) {
				if (allcar.get(i).getId().getCar().getCarnum().equals(car.get(j).getCarnum())) {
					car.remove(j);
				}
			}
...
<body onload="showTime()">
	<div class="main-content">
		...
		<div class="center-content">
			<div class="form-div">
				<form action="updateTrainCar" method="post">
					<table>
						<tr style="text-align: center;">
							<th colspan="2"><h2 style="text-shadow:1px 1px 1px black;">教练配车修改</h2></th>
						</tr>
						<tr>

							<th align="center" style="height: 30px; "><c:forEach
									items="${list }" var="list">
   					教练编号：	<input type="text" name="cnum"
										value="${list.id.coaches.num}" readonly="readonly" />

								</c:forEach></th>
						</tr>
						<tr>
							<th align="center" style="height: 30px;">原车辆： <c:forEach
									items="${list }" var="list">
									<input type="text" value="${list.id.car.carnum}"
										readonly="readonly" />
								</c:forEach>
						...
						<tr>
							<th align="center">请选择车辆：<select name="carnum">
